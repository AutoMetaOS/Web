...
const keyCalc = () => {
    const dt = btoa( ~~( ( Date.now() % 1e9 ) / 1e3 ) );
}

const keySplit = ( dt ) => {
    const date = ( ~~( Date.now() / 1e10 ) + dt ) * 1e3;
}


const auth = {
    isClear: () => {
        // RECALCULATE SYSTEM KEY
        // UNTANGLE BASE64 and SYSKEY
        // TAKE BASE64 and GET NORMAL KEY
        // GET NORKEY DATE and CALCULATE TIME and MAINKEY
        // SEND MAINKEY FOR VERIFICATION
        // IF INVALID F else GO
        localStorage.getItem( 'nukeKey' )
    },
    getKey: () => {
        // Contact Nuke API and get Key
        // If entered key match
        // Return verified and change command
        // Store SOMETHING UNIQUE NON REPLICABLE
    }
}